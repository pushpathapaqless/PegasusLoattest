...
class AddWalkInsFromWebKioskAndCheckStatus extends Simulation{
	...
	val walkInStatus = http("walkInStatus")
				.get(s"/organizations/${organizationId}/locations/${locationId}/walkins/#{walkInId}")
				.header(HttpHeaderNames.ContentType, "application/json")
				.body(StringBody("""{
	...
	val scn = scenario("Create WalkIn & Check Status")
		.exec(session => {
